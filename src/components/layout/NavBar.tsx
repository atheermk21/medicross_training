...
function NavBar() {
  ...
  return (
    <nav className="sticky   bg-[var(--bg-primary)]    py-8 flex justify-between items-center">
    
      <div className="container gap-3  mx-auto flex justify-between items-center p-4">
   
     <div className=" flex-shrink-0  flex items-center gap-6 flex h-11 text-[var(--text-primary)]" > <img className="h-13 w-auto" src={logo}  alt="logo" /></div>
     
      <div className="hidden md:flex items-center justify-center  bg-white/10 font-medium rounded-2xl h-18 w-250 text-[var(--text-primary)]">
      <ul className="hidden md:flex gap-6 font-semibold ml-20" >
     <li><Link to={ROUTES.HOME} className="flex items-center gap-1 ">Home </Link></li>
      <li><Link to={ROUTES.ABOUT} className="flex items-center gap-1 ">About </Link></li>
      <li><Link to={ROUTES.SERVICES} className="flex items-center gap-1 ">Services</Link></li>
      <li><Link to={ROUTES.DOCTORS} className="flex items-center gap-1">Doctors </Link></li>
      <li><Link to={ROUTES.CONTACT} className="flex items-center gap-1 ">Contact </Link></li>
      </ul>
      <span className="cursor-pointer flex items-center gap-2 mr-50"></span><IoSearchSharp size={22}/> </div>
          <div className="hidden md:flex">
<Button  to="/doctors"  className=" 
       bg-[var(--color-primary)] h-15 rounded-xl font-medium hover:bg-yellow-300 " variant="primary"><span>Book An Appointment</span><FiArrowRight/></Button>
      </div></div>
      <div className="">
  ...
  {isMenuOpen &&(
    <div className="fixed inset-0 z-50  flex flex-col p-6 md:hidden bg-[var(--bg-secondary)]">
      ...
      <div className="flex justify-center mb-8 bg-[var(--bg-secondary)] "><img src={logo2} alt="logo" /></div>
      ...
      <ul className="flex flex-col bg-[var(--bg-secondary)] gap-6 w-full  font-bold">
